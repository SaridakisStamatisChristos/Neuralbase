    ...
    for _ in 0..num_rows {
        ...
        for col_idx in 0..num_cols {
            ...
            match col_tags[col_idx] {
                TAG_INT32 => {
                    int32_data[col_idx].push(if is_null { None } else {
                        let v = read_i32(bytes, &mut pos)?;
                        Some(v)
                    });
                }
                TAG_INT64 => {
                    int64_data[col_idx].push(if is_null { None } else {
                        let v = read_i64(bytes, &mut pos)?;
                        Some(v)
                    });
                }
                TAG_FLOAT64 => {
                    float64_data[col_idx].push(if is_null { None } else {
                        let v = read_f64(bytes, &mut pos)?;
                        Some(v)
                    });
                }
                TAG_DATE32 => {
                    date32_data[col_idx].push(if is_null { None } else {
                        let v = read_i32(bytes, &mut pos)?;
                        Some(v)
                    });
                }
                TAG_UTF8 => {
                    utf8_strs[col_idx].push(if is_null { None } else {
    ...
    let columns: Vec<(String, ColumnVector)> = col_names
        ...
        .map(|(i, (name, &tag))| {
            let cv = match tag {
                ...
                TAG_UTF8    => {
                    let opts: Vec<Option<&str>> = utf8_strs[i]
                        .iter()
                        .map(|o| o.as_deref())
                        .collect();
...
mod tests {
    ...
    fn two_row_batch() -> RecordBatch {
        RecordBatch::new(vec![
            ...
            ("price".into(), ColumnVector::Float64(vec![Some(9.99), None])),
            ("label".into(), ColumnVector::Utf8(
                Utf8Column::from_options(vec![Some("foo"), Some("bar")]),
            )),
        ])
        .unwrap()
    }
    ...
    fn magic_present_in_encoded_output() {
        ...
        assert_eq!(&enc[0..3], &[0x4E, 0x42, 0x02], "must start with NB-v2 magic");
    }
    ...
    fn encode_decode_two_row_roundtrip() {
        ...
        // id column
        let ColumnVector::Int32(ids) = got.column("id").unwrap() else { panic!("type"); };
        assert_eq!(ids[0], Some(1));
        assert_eq!(ids[1], Some(2));

        // price column — second row is NULL
        let ColumnVector::Float64(prices) = got.column("price").unwrap() else { panic!("type"); };
        assert!((prices[0].unwrap() - 9.99).abs() < 1e-12);
        assert!(prices[1].is_none());

        // label column
        let ColumnVector::Utf8(labels) = got.column("label").unwrap() else { panic!("type"); };
    ...
    fn encode_decode_all_null_column() {
        let batch = RecordBatch::new(vec![
            ("x".into(), ColumnVector::Int64(vec![None, None, None])),
        ])
        .unwrap();
        let got = decode_batch(&encode_batch(&batch).unwrap()).unwrap();
        let ColumnVector::Int64(vals) = got.column("x").unwrap() else { panic!(); };
    ...
    fn decode_rejects_wrong_magic() {
        assert!(decode_batch(&[0x4E, 0x42, 0x01, 0, 0, 0, 0, 0, 0]).is_none(), "v1 must be rejected");
    ...
    fn date32_roundtrip() {
        let batch = RecordBatch::new(vec![
            ("d".into(), ColumnVector::Date32(vec![Some(19940101), None, Some(19960630)])),
        ])
        .unwrap();
        let got = decode_batch(&encode_batch(&batch).unwrap()).unwrap();
        let ColumnVector::Date32(dates) = got.column("d").unwrap() else { panic!(); };
    ...
    fn int64_roundtrip() {
        let batch = RecordBatch::new(vec![
            ("big".into(), ColumnVector::Int64(vec![Some(i64::MAX), Some(-1), None, Some(0)])),
        ])
        .unwrap();
        let got = decode_batch(&encode_batch(&batch).unwrap()).unwrap();
        let ColumnVector::Int64(vals) = got.column("big").unwrap() else { panic!(); };
    ...
    fn typed_codec_more_compact_than_string_codec() {
        ...
        let batch = RecordBatch::new(vec![
            ("l_orderkey".into(),      ColumnVector::Int64(vec![Some(1234567)])),
            ("l_extendedprice".into(), ColumnVector::Float64(vec![Some(12345.67)])),
        ...
        assert!(
            encoded.len() < 150,
            "typed codec should be compact: {} bytes", encoded.len()
        );
    }
