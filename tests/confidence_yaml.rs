...
fn distributed_claim_tracks_membership_identity_recovery_and_reads_without_overclaiming_production() {
    ...
    assert_eq!(scope["strong_reads_require_current_leader"].as_bool(), Some(true));
    assert_eq!(
    ...
    assert_eq!(scope["automatic_strong_read_routing"].as_bool(), Some(false));
    assert_eq!(scope["readindex_or_lease_optimization"].as_bool(), Some(false));
