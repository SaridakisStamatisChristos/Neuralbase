...
//! commit point before Raft may advance over prior-term entries. This regression
//! reproduces the condition that previously made the Phase-4 process identity test
//! intermittent: the identity command was quorum-durable, but one restarted node
//! could wait forever for an unrelated later write before applying it.
